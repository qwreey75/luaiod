...
function module.progress(per,size)
...
function module.progressWithBackground(per,size)
...
function module.progressWithFrame(title,per,size)
...
function module.progressWithFrameAndBackground(title,per,size)
	size = math.max(size,8);
	local titlebarSize = (size-2)/2-(utf8.len(title)/2);
	io.write(
		"\n┌",string.rep("─",math.floor(titlebarSize+0.5)),
